...
class Interval:
    ...
    Класс Interval - интервальная арифметика с управляемой точностью
            Поля:
                ...
                [[Операция деления, которая должна вернуть список из двух интервалов при 1, вернёт [-Inf, +Inf] при 0]]
                
                precision (int): точность результата, необходимое количество значащих цифр после запятой        | Default: 10
                [[Точность хранения и вывода чисел]]
                
                calcprecision (int): точность вычислений: с какой точностью работают операции Decimal           | Default: 50
                [[Должна быть достаточно большой, чтобы операции были математически корректны]]
                [[При выборе необходимо ориентироваться на ОБЩЕЕ количество значащих цифр в числах]]
                [[В большинстве случаев достаточно calcprecision = max(50, precision * 4 + 10, колзначцифр * 4 + 10)]]
                [[Оказывает наибольшее влияние на скорость исполнения]]
            ...
            Методы:
                ...
                Interval sqrt (self): возвращает __pow__ (self, 0.5) для совместимости с numpy
                ...
                Interval intersect (self, Interval other): возвращает интервал - пересечение двух заданных
                Interval union (self, Interval other): возвращает интервал - объединение двух заданных
    ...
    def __init__(self, x):
        ...
        '''
        setcontext(Context(prec=max(Interval.calcprecision, Interval.precision + 1), Emax=MAX_EMAX, Emin=MIN_EMIN, traps=[]))
        self.x = [0, 0]
    ...
    def mid(self):
        ...
        if (self.x[0] != Decimal("-Inf") and self.x[1] != Decimal("Inf")):
            middle = (Decimal("0.5") * (self.x[0] + self.x[1])).quantize(Decimal(quantizestring(self.precision)), rounding=ROUND_HALF_EVEN)
        return middle
    ...
    def intersect(self, other):
        ...
        Пересечение интервалов
                ...
                Возвращаемое значение:
                        result (Interval): результат операции пересечения;
        '''
        ointerval = Interval.valueToInterval(other)
        ninterval = Interval(self.x)
        setcontext(Context(prec=max(Interval.calcprecision, Interval.precision + 1), Emax=MAX_EMAX, Emin=MIN_EMIN, traps=[]))
        getcontext().rounding = ROUND_FLOOR
        ninterval.x[0] = max(self.x[0], ointerval.x[0])
        getcontext().rounding = ROUND_CEILING
        ninterval.x[1] = min(self.x[1], ointerval.x[1])
        if (ninterval.x[1] < ninterval.x[0]):
            ninterval.x[0] = Decimal('-INF')
            ninterval.x[1] = Decimal('-INF')
    ...
    def union(self, other):
        ...
        Объединение интервалов
                Параметры:
                        other (...): объект, из которого можно создать объект класса Interval;
                Возвращаемое значение:
                        result (Interval): результат операции объединения;
        '''
        ointerval = Interval.valueToInterval(other)
        ninterval = Interval(self.x)
        setcontext(Context(prec=max(Interval.calcprecision, Interval.precision + 1), Emax=MAX_EMAX, Emin=MIN_EMIN, traps=[]))
        getcontext().rounding = ROUND_FLOOR
        ninterval.x[0] = min(self.x[0], ointerval.x[0])
        getcontext().rounding = ROUND_CEILING
        ninterval.x[1] = max(self.x[1], ointerval.x[1])
        ninterval.__correctize()
        return ninterval
    ...
    def sqrt(self):
        return Interval.__pow__(self, Decimal("0.5"))

    def __eq__(self, other):
        ointerval = Interval.valueToInterval(other)
        setcontext(Context(prec=max(Interval.calcprecision, Interval.precision + 1), Emax=MAX_EMAX, Emin=MIN_EMIN, traps=[]))
        if self.x[0] == ointerval.x[0] and self.x[1] == ointerval.x[1]:
            return True
        return False
    ...
    def conjugate(self):
        return Interval.valueToInterval(self)
    ...
    def valueToInterval(expr):
        '''
        Создание объекта Interval из int, float, str, Decimal или списка из хотя бы двух объектов
                Параметры:
    ...
    def sin(x):
        ...
        x = Interval.valueToInterval(x)
        setcontext(Context(prec=max(Interval.calcprecision, Interval.precision + 1), Emax=MAX_EMAX, Emin=MIN_EMIN, traps=[]))
        ed = Decimal(quantizestring(Interval.precision)).quantize(Decimal(quantizestring(Interval.precision)), rounding=ROUND_CEILING)
        getcontext().rounding = ROUND_FLOOR
        ...
        if ((x[0] - pi05) / pi2).quantize(Decimal("1"), rounding=ROUND_CEILING) <= ((x[1] - pi05) / pi2).quantize(Decimal("1"), rounding=ROUND_FLOOR):
            b = Decimal("1")
        else:
            b = max(yru)
        if ((x[0] + pi05) / pi2).quantize(Decimal("1"), rounding=ROUND_CEILING) <= ((x[1] + pi05) / pi2).quantize(Decimal("1"), rounding=ROUND_FLOOR):
    ...
    def cos(x):
        ...
        pi2 = 2 * decpi()
        if (x[0] / pi2).quantize(Decimal("1"), rounding=ROUND_CEILING) <= (x[1]/pi2).quantize(Decimal("1"), rounding=ROUND_FLOOR):
            b = Decimal("1")
        else:
            b = max(yru)
        if ((x[0] - decpi()) / pi2).quantize(Decimal("1"), rounding=ROUND_CEILING) <= ((x[1] - decpi()) / pi2).quantize(Decimal("1"), rounding=ROUND_FLOOR):
            a = Decimal("-1")
        else:
            a = min(yrd)
        return Interval([a, b])
    ...
    def positive(x):
        '''
        Отбрасывание отрицательной части интервала
                Параметры:
                        x (...): объект, из которого можно создать объект класса Interval;
                Возвращаемое значение:
                        result (Interval): объект класса Interval, строго положительный;
                            Точность: зависит от параметров Interval.
                            Округление: внешнее расширяющее.
        '''
        x = Interval.valueToInterval(x)
    ...
    def negative(x):
        '''
        Отбрасывание положительной части интервала
                Параметры:
                        x (...): объект, из которого можно создать объект класса Interval;
                Возвращаемое значение:
                        result (Interval): объект класса Interval, строго отрицательный;
                            Точность: зависит от параметров Interval.
                            Округление: внешнее расширяющее.
        '''
        x = Interval.valueToInterval(x)
    ...
    def ln(x):
        '''
        Натуральный логарифм положительного интервала
                Параметры:
                        x (...): объект, из которого можно создать объект класса Interval;
                Возвращаемое значение:
    ...
    def getMid(x):
        '''
        Середина матрицы интервалов
                Параметры:
                        x (numpy.array): матрица интервалов;
                Возвращаемое значение:
                        result (numpy.array): середина матрицы;
        '''
        n, m = x.shape
        A = np.empty((n, m))
        for i in range(n):
    ...
    @staticmethod
    def getVectIntersect(x, y):
        '''
        Пересечение вектора интервалов
                Параметры:
                        x (numpy.array): вектор интервалов;
                        y (numpy.array): вектор интервалов;
                Возвращаемое значение:
                        result (numpy.array): пересечение векторов интервалов;
        '''
        A = []
        for i in range(len(x)):
            A.append(x[i].intersect(y[i]))
        return np.array(A)

    @staticmethod
    def vectorWidth(x):
        '''
        Ширина вектора интервалов
                Параметры:
                        x (numpy.array): вектор интервалов;
                Возвращаемое значение:
                        result (Decimal): ширина вектора;
        '''
        imaxwidth = 0
        maxwidth = 0
        for i in range(len(x)):
            if (x[i].width() > maxwidth):
                maxwidth = x[i].width()
                imaxwidth = i
        return imaxwidth, maxwidth
    ...
    def inverseDiag(x):
        ...
        Середина матрицы интервалов
                ...
                Возвращаемое значение:
                        result (numpy.array): середина матрицы;
        '''
        mim, Interval.multiintervalmode = Interval.multiintervalmode, 0
        n, n = x.shape
        A = np.zeros((n, n), dtype=object)
        for i in range(n):
            A[i][i] = Interval.valueToInterval(1) / x[i][i]
        Interval.multiintervalmode = mim
        return A
